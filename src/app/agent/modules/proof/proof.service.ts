...
export class ProofService {
  ...
  */
  async sendProposal(
    ...
    try {
      const res = presExId
        ? await request
            .post(`${apiUrl}${segment}records/${presExId}/send-request`)
            .send(data)
        : await request.post(`${apiUrl}${segment}send-proposal`).send(data);
      if (res.status === 200) return res.body;
  ...
  async sendProof(data: any): Promise<IProofRecordsResponse> {
    try {
      const res = await request
        .post(`${apiUrl}${segment}records/send-presentation`)
        .send(data);
      if (res.status === 200) return res.body;
  ...
  async sendProofVerification(
    ...
    try {
      const res = await request.post(
        `${apiUrl}${segment}records/${presExId}/verify-presentation`
      );
      if (res.status === 200) return res.body;
  ...
  */
  async removeProofRecord(presExId: string) {
    try {
      const res = await request.post(
        `${apiUrl}${segment}records/${presExId}/remove`
      );
      if (res.status === 200) return res.body;
